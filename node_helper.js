...
// ═══════════════════════════════════════════════════════════════════════════════════
// RECURRING TASK INSTANCE LOCKING
// ═══════════════════════════════════════════════════════════════════════════════════
// 
// PROBLEM: Multiple code paths can call ensureRecurringInstancesUpToToday() concurrently:
//   - broadcastTasks() from 16+ locations
//   - GET /api/tasks endpoint (admin portal)
//   - PUT /api/tasks/:id endpoint (task completion)
// 
// Without synchronization, a race condition can occur:
//   1. Path A calls ensureRecurringInstancesUpToToday(), builds dateSet
//   2. Path B calls ensureRecurringInstancesUpToToday() before Path A completes
//   3. Both check dateSet (now stale), calculate same candidateDate
//   4. Both call createRecurringInstanceFromTask() and push to tasks array
//   5. Result: DUPLICATE tasks for the same date
//
// SOLUTION: Promise-based queue lock
//   - When a call acquires the lock, it gets a promise
//   - Other callers await that promise, then get their own
//   - This serializes access, preventing concurrent execution
//   - All mutations are synchronous, so lock is simple and effective
//
let recurringTaskLock = Promise.resolve();

/**
 * withRecurringTaskLock(fn)
 * 
 * Wrapper function that serializes access to recurring task operations.
 * Ensures that only one caller can execute ensureRecurringInstancesUpToToday() at a time.
 * 
 * This uses a promise-based queue lock pattern:
 *   - Each caller awaits the current lock promise
 *   - Then creates a new lock promise that subsequent callers will await
 *   - Prevents race conditions where multiple paths try to create the same recurring instance
 * 
 * @param {Function} fn - Async function to execute within the lock
 * @returns {Promise} - Result of fn() after lock is acquired
 * 
 * Usage:
 *   const result = await withRecurringTaskLock(() => ensureRecurringInstancesUpToToday());
 */
async function withRecurringTaskLock(fn) {
  // Wait for any previous lock holder to finish
  await recurringTaskLock;
  
  // Create a new lock promise that others will wait for
  let releaseLock;
  recurringTaskLock = new Promise(resolve => {
    releaseLock = resolve;
  });
 ...
 * 
 * Master orchestrator that ensures all recurring instances are up-to-date, then broadcasts
 * the complete task list to all connected clients via socket notifications.
 * 
 * This function is called from many locations (16+ entry points), and can be invoked
 * concurrently by different code paths. To prevent recurring task duplication race conditions,
 * it calls ensureRecurringInstancesUpToToday() WITHIN THE LOCK.
 * 
 * Flow:
 *   1. (LOCKED) Call ensureRecurringInstancesUpToToday() to create missing instances
 *   2. Sort tasks by deletion status and order
 *   3. Update people levels based on current task completion
 *   4. Send socket notifications with updated data to all connected clients
 *   5. Persist changes to disk via saveData()
 * 
 * @param {Object} helper - NodeHelper instance (provides sendSocketNotification)
 * @returns {Boolean} - True if data was saved successfully, false otherwise
 * 
 * NOTE: This function is now async to support the lock.
 * Callers should use await when calling this function, or fire-and-forget if they
 * don't need the return value.
 * 
 * Example:
 *   // Broadcast immediately (don't wait):
 *   broadcastTasks(this);
 *   
 *   // Wait for broadcast to complete with lock held:
 *   await broadcastTasks(this);
 */
async function broadcastTasks(helper) {
  ...
  const autoGenerated = await withRecurringTaskLock(() => ensureRecurringInstancesUpToToday());
  if (autoGenerated) {
 ...
 * 
 * MUST BE CALLED WITHIN withRecurringTaskLock() to prevent race conditions.
 * 
 * This function creates missing recurring task instances for all dates up to and including today.
 * It ensures that every recurring series has instances generated for all past dates that should
 * have been generated by now, maintaining a continuous schedule.
 * 
 * Algorithm:
 ...
 *   
 *   // RIGHT - serialized via lock:
 *   const created = await withRecurringTaskLock(() => ensureRecurringInstancesUpToToday());
 */
  ...
  socketNotificationReceived(notification, payload) {
    if (notification === "INIT_SERVER") {
      ...
      if (!this.server) {
        ...
        // Note: Fire-and-forget since we don't need to wait for completion
        broadcastTasks(this).catch(err => Log.error("broadcastTasks failed", err));
...
Return JSON only: {"action": "ACTION_NAME", "params": {...}, "response": "natural language response"}`;
  ...
  initServer(port) {
    ...
    app.delete("/api/people/:id", requireWrite, async (req, res) => {
    ...
    // NOTE: This endpoint is async to support the lock for ensureRecurringInstancesUpToToday().
    ...
    app.post("/api/tasks", requireTaskWrite, async (req, res) => {
      const now = new Date();
      ...
      const ok = await broadcastTasks(self);
      res.status(ok ? 201 : 500).json(ok ? newTask : { error: "Failed to save data" });
    });

    // Reorder tasks
    app.put("/api/tasks/reorder", requireTaskWrite, async (req, res) => {
      const ids = req.body;
    ...
    app.put("/api/tasks/:id", requireTaskWrite, async (req, res) => {
      const id   = parseInt(req.params.id, 10);
    ...
    app.delete("/api/tasks/:id", requireTaskWrite, async (req, res) => {
      const id = parseInt(req.params.id, 10);
      const task = tasks.find(t => t.id === id);
      if (!task) return res.status(404).json({ error: "Task not found" });
